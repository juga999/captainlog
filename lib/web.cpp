...
expected<bool, std::string> WebServer::handle_get_js_request(struct evhttp_request* req, std::string&& css_file)
{
    std::string resource_path = m_web_root + "/js/" + css_file;
    ...
    bool cache)
{
    ...
    if (cache) {
        evhttp_add_header(evhttp_request_get_output_headers(req), "Cache-Control", "public, max-age=604800, immutable");
    } else {
